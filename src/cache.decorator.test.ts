...
describe('Cache Decorators', () => {
	...
	describe('Helpers', () => {
		describe('resolveCacheKey', () => {
			it('should return static string key as-is', async () => {
				const result = await resolveCacheKey('static:key', [
					{
						id: '123',
					},
				])
			...
			it('should resolve key from sync function using context', async () => {
				...
				const result = await resolveCacheKey(keyFn, [
					{
						id: '456',
					},
				])
		...
		describe('resolveCacheDelKeys', () => {
			...
			it('should resolve function returning string to array', async () => {
				...
				const result = await resolveCacheDelKeys(keyFn, [
					{
						id: '123',
					},
				])
	...
	describe('Cache.Del', () => {
		...
		it('should accept a static array of keys', async () => {
			await expect(
				resolveCacheDelKeys(
					[
						'a',
		...
		it('should log and swallow invalidation errors', async () => {
			const consoleError = vi
				.spyOn(console, 'error')
				.mockImplementation(() => undefined)
			vi.mocked(mockCacheService.del).mockRejectedValue(new Error('boom'))

			await expect(service.deleteStatic()).resolves.toBe('deleted')
			expect(consoleError).toHaveBeenCalledWith(
				'[Cache.Del] Invalidation failed:',
				expect.any(Error),
			)
	...
	describe('without CacheService on the instance', () => {
		it('should fall back to the original method', async () => {
			service.cacheService = undefined as unknown as CacheService

			await expect(service.getStatic()).resolves.toBe('value-from-db')
			expect(mockCacheService.get).not.toHaveBeenCalled()
		})
	})

	describe('Cache.Get set failure', () => {
		it('should log and swallow set errors', async () => {
			const consoleError = vi
				.spyOn(console, 'error')
				.mockImplementation(() => undefined)
			vi.mocked(mockCacheService.get).mockResolvedValue(null)
			vi.mocked(mockCacheService.set).mockRejectedValue(new Error('boom'))

			await expect(service.getStatic()).resolves.toBe('value-from-db')
			await vi.waitFor(() => {
				expect(consoleError).toHaveBeenCalledWith(
					'[Cache.Get] Set failed for key static:key:',
					expect.any(Error),
				)
			})
